(function initWaves() {
  class Grad {
    constructor(x, y, z) { this.x = x; this.y = y; this.z = z; }
    dot2(x, y) { return this.x * x + this.y * y; }
  }

  class Noise {
    constructor(seed = 0) {
      this.grad3 = [
        new Grad(1,1,0), new Grad(-1,1,0), new Grad(1,-1,0), new Grad(-1,-1,0),
        new Grad(1,0,1), new Grad(-1,0,1), new Grad(1,0,-1), new Grad(-1,0,-1),
        new Grad(0,1,1), new Grad(0,-1,1), new Grad(0,1,-1), new Grad(0,-1,-1)
      ];
      this.p = [
        151,160,137,91,90,15,131,13,201,95,96,53,194,233,7,225,140,36,103,30,69,142,8,99,37,240,
        21,10,23,190,6,148,247,120,234,75,0,26,197,62,94,252,219,203,117,35,11,32,57,177,33,88,
        237,149,56,87,174,20,125,136,171,168,68,175,74,165,71,134,139,48,27,166,77,146,158,231,83,
        111,229,122,60,211,133,230,220,105,92,41,55,46,245,40,244,102,143,54,65,25,63,161,1,216,
        80,73,209,76,132,187,208,89,18,169,200,196,135,130,116,188,159,86,164,100,109,198,173,186,
        3,64,52,217,226,250,124,123,5,202,38,147,118,126,255,82,85,212,207,206,59,227,47,16,58,
        17,182,189,28,42,223,183,170,213,119,248,152,2,44,154,163,70,221,153,101,155,167,43,172,9,
        129,22,39,253,19,98,108,110,79,113,224,232,178,185,112,104,218,246,97,228,251,34,242,193,
        238,210,144,12,191,179,162,241,81,51,145,235,249,14,239,107,49,192,214,31,181,199,106,157,
        184,84,204,176,115,121,50,45,127,4,150,254,138,236,205,93,222,114,67,29,24,72,243,141,128,
        195,78,66,215,61,156,180
      ];
      this.perm = new Array(512);
      this.gradP = new Array(512);
      this.seed(seed);
    }
    seed(seed) {
      if (seed > 0 && seed < 1) seed *= 65536;
      seed = Math.floor(seed);
      if (seed < 256) seed |= seed << 8;
      for (let i = 0; i < 256; i++) {
        const v = i & 1 ? this.p[i] ^ (seed & 255) : this.p[i] ^ ((seed >> 8) & 255);
        this.perm[i] = this.perm[i + 256] = v;
        this.gradP[i] = this.gradP[i + 256] = this.grad3[v % 12];
      }
    }
    fade(t) { return t * t * t * (t * (t * 6 - 15) + 10); }
    lerp(a, b, t) { return (1 - t) * a + t * b; }
    perlin2(x, y) {
      let X = Math.floor(x), Y = Math.floor(y);
      x -= X; y -= Y;
      X &= 255; Y &= 255;
      const n00 = this.gradP[X + this.perm[Y]].dot2(x, y);
      const n01 = this.gradP[X + this.perm[Y + 1]].dot2(x, y - 1);
      const n10 = this.gradP[X + 1 + this.perm[Y]].dot2(x - 1, y);
      const n11 = this.gradP[X + 1 + this.perm[Y + 1]].dot2(x - 1, y - 1);
      const u = this.fade(x);
      return this.lerp(this.lerp(n00, n10, u), this.lerp(n01, n11, u), this.fade(y));
    }
  }

  const ambientBg = document.querySelector('.ambient-bg');
  if (!ambientBg) return;

  const canvas = document.createElement('canvas');
  canvas.className = 'waves-canvas';
  // Insert after bg-wash so waves sit above the base layer but below sweeps/scanlines
  const bgWash = ambientBg.querySelector('.bg-wash');
  if (bgWash && bgWash.nextSibling) {
    ambientBg.insertBefore(canvas, bgWash.nextSibling);
  } else {
    ambientBg.appendChild(canvas);
  }

  const ctx = canvas.getContext('2d');
  const noise = new Noise(Math.random());

  const cfg = {
    lineColor: 'rgba(255, 255, 255, 0.22)',
    waveSpeedX: 0.02,
    waveSpeedY: 0.01,
    waveAmpX: 40,
    waveAmpY: 20,
    friction: 0.9,
    tension: 0.01,
    maxCursorMove: 120,
    xGap: 14,
    yGap: 36,
  };

  let W = 0, H = 0, lines = [];
  const mouse = { x: -10, y: 0, lx: 0, ly: 0, sx: 0, sy: 0, v: 0, vs: 0, a: 0, set: false };

  function setSize() {
    W = canvas.width = window.innerWidth;
    H = canvas.height = window.innerHeight;
  }

  function buildLines() {
    lines = [];
    const oW = W + 200, oH = H + 30;
    const { xGap, yGap } = cfg;
    const cols = Math.ceil(oW / xGap);
    const rows = Math.ceil(oH / yGap);
    const x0 = (W - xGap * cols) / 2;
    const y0 = (H - yGap * rows) / 2;
    for (let i = 0; i <= cols; i++) {
      const pts = [];
      for (let j = 0; j <= rows; j++) {
        pts.push({
          x: x0 + xGap * i,
          y: y0 + yGap * j,
          wave: { x: 0, y: 0 },
          cursor: { x: 0, y: 0, vx: 0, vy: 0 }
        });
      }
      lines.push(pts);
    }
  }

  function movePoints(t) {
    const { waveSpeedX, waveSpeedY, waveAmpX, waveAmpY, friction, tension, maxCursorMove } = cfg;
    for (const pts of lines) {
      for (const p of pts) {
        const n = noise.perlin2((p.x + t * waveSpeedX) * 0.002, (p.y + t * waveSpeedY) * 0.0015) * 12;
        p.wave.x = Math.cos(n) * waveAmpX;
        p.wave.y = Math.sin(n) * waveAmpY;

        const dx = p.x - mouse.sx, dy = p.y - mouse.sy;
        const dist = Math.hypot(dx, dy);
        const reach = Math.max(175, mouse.vs);
        if (dist < reach) {
          const s = 1 - dist / reach;
          const f = Math.cos(dist * 0.001) * s;
          p.cursor.vx += Math.cos(mouse.a) * f * reach * mouse.vs * 0.00065;
          p.cursor.vy += Math.sin(mouse.a) * f * reach * mouse.vs * 0.00065;
        }
        p.cursor.vx += (0 - p.cursor.x) * tension;
        p.cursor.vy += (0 - p.cursor.y) * tension;
        p.cursor.vx *= friction;
        p.cursor.vy *= friction;
        p.cursor.x += p.cursor.vx * 2;
        p.cursor.y += p.cursor.vy * 2;
        p.cursor.x = Math.min(maxCursorMove, Math.max(-maxCursorMove, p.cursor.x));
        p.cursor.y = Math.min(maxCursorMove, Math.max(-maxCursorMove, p.cursor.y));
      }
    }
  }

  function pos(p, cur = true) {
    return {
      x: Math.round((p.x + p.wave.x + (cur ? p.cursor.x : 0)) * 10) / 10,
      y: Math.round((p.y + p.wave.y + (cur ? p.cursor.y : 0)) * 10) / 10,
    };
  }

  function draw() {
    ctx.clearRect(0, 0, W, H);
    ctx.beginPath();
    ctx.strokeStyle = cfg.lineColor;
    ctx.lineWidth = 1;
    for (const pts of lines) {
      const start = pos(pts[0], false);
      ctx.moveTo(start.x, start.y);
      for (let i = 0; i < pts.length; i++) {
        const isLast = i === pts.length - 1;
        const a = pos(pts[i], !isLast);
        ctx.lineTo(a.x, a.y);
        if (isLast) {
          const b = pos(pts[pts.length - 1], false);
          ctx.moveTo(b.x, b.y);
        }
      }
    }
    ctx.stroke();
  }

  function tick(t) {
    mouse.sx += (mouse.x - mouse.sx) * 0.1;
    mouse.sy += (mouse.y - mouse.sy) * 0.1;
    const dx = mouse.x - mouse.lx, dy = mouse.y - mouse.ly;
    mouse.v = Math.hypot(dx, dy);
    mouse.vs += (mouse.v - mouse.vs) * 0.1;
    mouse.vs = Math.min(100, mouse.vs);
    mouse.lx = mouse.x;
    mouse.ly = mouse.y;
    mouse.a = Math.atan2(dy, dx);
    movePoints(t);
    draw();
    requestAnimationFrame(tick);
  }

  function updateMouse(x, y) {
    mouse.x = x; mouse.y = y;
    if (!mouse.set) {
      mouse.sx = x; mouse.sy = y;
      mouse.lx = x; mouse.ly = y;
      mouse.set = true;
    }
  }

  window.addEventListener('resize', () => { setSize(); buildLines(); });
  window.addEventListener('mousemove', e => updateMouse(e.clientX, e.clientY));
  window.addEventListener('touchmove', e => {
    updateMouse(e.touches[0].clientX, e.touches[0].clientY);
  }, { passive: false });

  setSize();
  buildLines();
  requestAnimationFrame(tick);
})();
